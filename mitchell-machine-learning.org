...
*** 1.5
 ...
 # <<wikipedia-strategy>>
    ...
    #+BEGIN_SRC scheme :tangle tic-tac-toe.scm :shebang #!/usr/bin/env chicken-scheme
      ...
      ;;; Putting triplet first would allow you to use many boards.
      (define (first-empty-space board triplet)
        (find (cute empty? board <>) triplet))
      
      (define (winning-triplet? player? triplet board)
        (let ((non-player-marks
               (filter (cute (complement player?) board <>) triplet)))
      ...
      (define (winning-spaces player? board)
        (fold-triplet
         (lambda (triplet winning-spaces)
           (if (winning-triplet? player? triplet board)
               (cons (first-empty-space board triplet) winning-spaces)
               winning-spaces))
         '()
         board))
